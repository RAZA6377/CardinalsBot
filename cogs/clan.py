...
class ClanManager:
    ...
    async def update_score(
        self, interaction: discord.Interaction, clan_name: str, option: str, points: int
    ):
        data = self.get_data()
        ...
        clan_thread = await interaction.guild.fetch_channel(
            int(data[clan_name]["clan_thread_id"])
        )
        await clan_thread.send(
            f"{clan_leader.mention}\n```Updated clan Points to {points}\nTotal = {data[clan_name]['points']}```"
        )
        return (
            "success",
            f"Updated **{clan_name}** Points to {data[clan_name]['points']}",
        )
    ...
    async def create(
        ...
        if role == "Leader":
            ...
            clan_thread = await channel.create_thread(
                name=f"{clan_name} Clan", invitable=False
            )
            clan_role = await user.guild.create_role(name=f"[Clan] {clan_name}")
            await clan_thread.add_user(user)
            await user.add_roles(clan_role)
            await clan_thread.send(
                f"This is a discussion thread for **{clan_name}**.\n**Clan Leader** : {user.mention}"
            )
            data[clan_name] = {
    ...
    async def delete(self, user: discord.Member):
        ...
        if role == "Member":
            return (
                "error",
                f"You are member of {clan} clan. You don't have permission to delete",
            )
        elif role is None:
...
class ClanMenu(Select):
    def __init__(self):
        ...
        options = [
            discord.SelectOption(label=clan_option, value=clan_value)
            for clan_option, clan_value in clan_menu_options.items()
        ]

        super().__init__(
            placeholder="Select a clan option",
            options=options,
            custom_id="clan:clanmenu",
        )
    ...
    async def callback(self, interaction: discord.Interaction):
            ...
            try:
                ...
                if role == "Leader":
                    await interaction.response.send_message(
                        f"You already own {clan_name} clan", ephemeral=True
                    )
                    return
                elif role == "Member":
                    await interaction.response.send_message(
                        f"You are already a member of {clan_name} clan", ephemeral=True
                    )
                    return
                else:
        ...
        # Clan List
            ...
            embed = discord.Embed(
                ...
                description="A list of available clans",
                color=0x00FFFF,
            )
            if data != {}:
                ...
                await interaction.response.send_message(
                    "```No Available Clans Found.```", ephemeral=True
                )
                return
...
class ConfirmButton(Button):
    def __init__(self, on_confirm):
        super().__init__(
            ...
            emoji="<a:tick:1546932427103150170>",
        )
...
class DenyButton(Button):
    def __init__(self, on_deny):
        super().__init__(
            ...
            emoji="<:Cross69:1546933008274165914>",
        )
...
class ConfirmView(discord.ui.View):
    def __init__(self, on_confirm, on_deny):
...
class JoinClanModal(Modal):
    def __init__(self):
        ...
        options = [
            discord.SelectOption(label=clan_name, value=clan_name)
            for clan_name, clan_data in self.clan_manager.get_data().items()
        ]

        self.clan_select = Select(
            placeholder="Select a clan", options=options, required=True
        )
        self.add_item(self.aid)
        self.add_item(
            discord.ui.Label(text="Select a clan", component=self.clan_select)
        )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            clan_thread = await interaction.guild.fetch_channel(
                clan_info["clan_thread_id"]
            )
            print(clan_thread)
            embed = discord.Embed(
                title="Clan Join Request",
                description=f"{interaction.user.name} Applied for your clan.\n```Username : {interaction.user.name}\nDiscord Id : {interaction.user.id}\nBs Aid : {self.aid_value}```",
                color=0x00FFFF,
            )

            async def on_approval_accept(interaction: discord.Interaction):
                if interaction.user.id == clan_leader.id:
                    user_add = await self.clan_manager.add_user(
                        self.applicant, self.aid_value, self.selected_clan
                    )
                    await interaction.response.edit_message(
                        content="**Approval Accepted**", view=None
                    )
                    if user_add == "Success":
                        try:
                            await self.applicant.send(
                                f"Congratulations! You are now a member of {self.selected_clan} Clan"
                            )
                        except:
                            pass
                    else:
                    ...
                    await interaction.response.send_message(
                        "You aren't clan leader", ephemeral=True
                    )

            async def on_approval_deny(interaction: discord.Interaction):
                if interaction.user.id == clan_leader.id:
                    await interaction.response.edit_message(
                        content="Approval Denied", view=None
                    )
                    try:
                        await self.applicant.send(
                            f"Your approval for {self.selected_clan} Clan has been declined. Try again"
                        )
                    except:
                        pass
                else:
            ...
            async def on_aid_confirm(interaction: discord.Interaction):
                await clan_thread.send(
                    ...
                    embed=embed,
                    view=ConfirmView(
                        on_confirm=on_approval_accept, on_deny=on_approval_deny
                    ),
                )
                await interaction.response.edit_message(
            ...
            if self.aid_value is None:
                await interaction.response.send_message(
                    "Invalid `a-id`. Try again", ephemeral=True
                )
                return
            aid_confirmation_embed = discord.Embed(
                title="Account Id Confirmation",
                description=f"**Tag**: `{self.aid_value['tag']}`\n**Account ID** : `{self.aid_value['id']}`\n**Created** : `{self.aid_value['create_time']}`",
                color=0x00FFFF,
            )
            await interaction.response.send_message(
                embed=aid_confirmation_embed,
                view=ConfirmView(on_confirm=on_aid_confirm, on_deny=on_aid_deny),
                ephemeral=True,
            )
...
class CreateClanModal(Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            if self.aid_value is None:
                await interaction.response.send_message(
                    "Invalid a-id entered", ephemeral=True
                )
                return

            if self.clan_name in data:
                await interaction.response.send_message(
                    "`A clan already exist with this name. Try something diffrent.`",
                    ephemeral=True,
                )
                return
            for clan_name, clan_data in data.items():
                for player in clan_data["players"]:
                    if self.aid_value["id"] == player["aid"]:
                        await interaction.response.send_message(
                            f"`This account id already exist in {clan_name} members list`",
                            ephemeral=True,
                        )
                        return

            status, result = await self.clan_manager.create(
                interaction.user, self.clan_name, self.aid_value["id"], self.clan_desc
            )
            if status == "error":
...
class ChallengeDashboard(LayoutView):
    def __init__(self, clan, opponent_clan):
        super().__init__(timeout=None)
        self.selected_players = []
        self.clan = clan
        self.oppo_clan = opponent_clan
        self.clan_manager = ClanManager()
    ...
    def _create_dashboard(self):
        ...
        options = [
            discord.SelectOption(
                label=str(player["name"]), value=int(player["user_id"])
            )
        ...
        # Show all selected players information in embed
        player_selected_text = ""
        for index, selected_player in enumerate(self.selected_players):
        ...
        selected_text = TextDisplay(
            f"```{'None' if not self.selected_players else player_selected_text}```"
        )
        ...
        # Confirm button with callbacks
        async def confirm_callback(interaction: discord.Interaction):
            # check if challenge data is empty if empty then its challenger team else its the opponent team data lets try.
            c_data = self.clan_manager.get_challenge_data()
            if self.selected_players == []:
                await interaction.response.send_message(
                    "Select atleast 1 player", ephemeral=True
                )
                ...
                for clan_player in self.clan_manager.get_data()[self.clan]["players"]:
                    if int(clan_player["user_id"]) == int(selected_player):
                        players_data["players"] = {
                            clan_player["user_id"]: {
                                "name": clan_player["name"],
                                "aid": clan_player["aid"],
                            }
                        }

            if c_data["challenge"] == {}:
                # challenger team data
                print(players_data)  # debug
                c_data["challenge"][self.clan] = players_data
                self.clan_manager.challenge_write(c_data)
                oppo_leader_id = self.clan_manager.get_clan(self.oppo_clan)["leader_id"]
                oppo_thread_id = self.clan_manager.get_clan(self.oppo_clan)[
                    "clan_thread_id"
                ]
            ...
            self.clan_manager.challenge_write(c_data)
            await interaction.response.send_message(
                "You successfully accepted challenge.."
            )

        confirm_row = ActionRow()
        confirm_row.add_item(ConfirmButton(confirm_callback))
        container.add_item(confirm_row)
...
class ClanCog(commands.Cog):
    ...
    def cog_load(self):
        self.bot.add_view(ClanDashboard(self.bot))

    async def clan_choices(self, interaction: discord.Interaction, current: str):
        data = self.clan_manager.get_data()
        return [
    ...
    async def update_score(
        ...
        try:
            status, result = await self.clan_manager.update_score(
                interaction, clan_name, option, points
            )
            if status == "error":
                await interaction.response.send_message(result, ephemeral=True)
    ...
    @app_commands.autocomplete(opponent_clan=clan_choices)
    async def challenge(self, interaction: discord.Interaction, opponent_clan: str):
        try:
            role, clan = self.clan_manager.get_user_info(interaction.user)
            challenge_data = self.clan_manager.get_challenge_data()
            print(challenge_data)

            if opponent_clan in clan:
                await interaction.response.send_messsge(
                    "You cannot challenge your own clan", ephemeral=True
                )
                return

            if role in ("Member", None):
                await interaction.response.send_message(
