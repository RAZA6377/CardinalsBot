...
class TicketManager:
    ...
    async def delete_ticket(self, channel: discord.TextChannel):
        try:
            ...
            channel_msgs = [messages async for messages in ticket.history(limit=None, oldest_first=True)]
            if ticket:
                ...
                await log_channel.send(content=f'Transcript of ticket : {ticket.name}',file=discord.File(f'{ticket.name}.txt'))
...
class CreateTicketModal(Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            reason = TextDisplay(f'> **Topic Name** : {self.topic.value}\n> **Reason** : ```{self.reason.value}```\n\n> <@&1539651469014335604> Assist them')
...
class CreateTicketButton(Button):
    def __init__(self):
        super().__init__(label='Create Ticket', style=discord.ButtonStyle.primary, emoji='<:tickets:1546595601679126528>')
    async def callback(self, interaction: discord.Interaction):
        try:
